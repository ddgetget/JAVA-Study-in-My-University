...
public class Fireworks extends Applet implements Runnable {
    ...
    public void run() {
        ...
        int P = (int) (Math.random() * Max_Patch * 3 / 4) // 设置烟花的斑点数
                + Max_Patch / 4 + 1;
        int L = (int) (Math.random() * Max_Length * 3 / 4) // 设置烟花可发射出的距离
                + Max_Length / 4 + 1;
        long S = (long) (Math.random() * 10000);// 设置随机产生烟火的数量
        boolean sleep;                // 设置体眠的标志
