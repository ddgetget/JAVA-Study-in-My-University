...
public class Root extends JFrame {
    ...
    public Root(String mgNo) {
        ...
        Image image = new ImageIcon(this.getClass().getResource("/ico.png")).getImage();
        this.setIconImage(image);
        this.setTitle("智达图书馆管理系统");
        //设置窗体首次出现的大小和位置--自动居中
...
//        this.setExtendedState(JFrame.MAXIMIZED_BOTH);//窗体全屏
    ...
    public void addTreeListener() {
        jt.addTreeSelectionListener(new TreeSelectionListener() {
            public void valueChanged(TreeSelectionEvent e) {
                ...
                if (cnv.value.equals("图书馆管理系统")) {//显示根结点信息
                    cl.show(jp, "root");
                }
                if (cnv.value.equals("学生用户管理")) {//显示学生用户管理界面
                    cl.show(jp, "stu");
                } else if (cnv.value.equals("图书管理")) {//显示图书管理界面
                    cl.show(jp, "bm");
                }
                if (cnv.value.equals("查询图书")) {//显示查询图书界面
                    cl.show(jp, "sb");
                } else if (cnv.value.equals("借阅预约图书")) {//显示借阅预约图书界面
                    cl.show(jp, "bb");
                } else if (cnv.value.equals("归还挂失图书")) {//显示归还挂失图书界面
                    cl.show(jp, "rb");
                } else if (cnv.value.equals("交纳罚款")) {//显示缴纳罚款界面
                    cl.show(jp, "et");
                } else if (cnv.value.equals("管理员管理")) {//显示管理员管理界面
                    cl.show(jp, "Manager");
                } else if (cnv.value.equals("退出")) {//显示退出界面
                    int i = JOptionPane.showConfirmDialog(Root.this, "是否退出系统?",
                            "消息", JOptionPane.YES_NO_OPTION);
                    if (i == JOptionPane.YES_OPTION) {//退出系统
                        System.exit(0);
                    }
                }
            }
    ...
    class NodeValue {
        ...
        public String getValue() {//value的Get方法
            return this.value;
        }
